...
import { Download, Play, Pause, Upload, Type, Eye, Focus, MousePointer2, Settings, Plus, Trash2, Sliders, Sparkles, Palette, Bold, Italic, Clock } from 'lucide-react';
...
export default function Editor() {
  ...
  const startRecording = () => {
    ...
    // Prefer VP9 for better color accuracy
  ...
  useEffect(() => {
    ...
    const render = (now_timestamp: number) => {
      ...
      // Color correction for export: Subtle adjustments to compensate for encoder vividness
      if (isRecording) {
        ctx.filter = 'saturate(0.96) contrast(0.98)';
      } else {
        ctx.filter = 'none';
      }
      ...
      if (currentChunk) {
        ...
        const subSize = Math.floor(baseSize * 0.7);
        const padding = 16;
        const lineSpacing = 8;
        ...
        const drawStyledLine = (
          ...
          const totalWidth = measureStyledSegmentsWidth(ctx, segments, fontName, baseFontSize, isDefaultBold);
          ...
          for (const seg of segments) {
            ...
            if (!seg.animation) {
              ...
              // Draw character-by-character for animation
        ...
        const mainMetricsWidth = measureStyledSegmentsWidth(ctx, segmentsMain, settings.fontFamily, baseSize, true);
        
        let subMetricsWidth = 0;
        let segmentsSub: TextSegment[] = [];
        if (currentChunk.subText) {
          segmentsSub = parseStyledText(currentChunk.subText);
          subMetricsWidth = measureStyledSegmentsWidth(ctx, segmentsSub, 'JetBrains Mono', subSize, false);
        }
        ...
        ctx.shadowBlur = 0;
        if (currentChunk.subText) {
          drawStyledLine(currentChunk.subText, yBaseline, 'JetBrains Mono', subSize, 'rgba(255, 255, 255, 0.9)', false);
          drawStyledLine(currentChunk.text, yBaseline - subSize - lineSpacing, settings.fontFamily, baseSize, settings.color, true);
        } else {
          drawStyledLine(currentChunk.text, yBaseline, settings.fontFamily, baseSize, settings.color, true);
        }
      }
    ...
    <div className="min-h-screen flex flex-col md:flex-row gap-6 p-6 max-w-7xl mx-auto items-stretch">
      ...
      <div className="w-full md:w-96 flex flex-col gap-4 bg-[#111] p-6 rounded-2xl border border-white/5 order-2 md:order-1 h-fit md:max-h-screen overflow-y-auto custom-scrollbar">
        <label className="group flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-white/10 rounded-xl cursor-pointer hover:border-red-500/50 hover:bg-red-500/5 transition-all">
          <Upload className="w-6 h-6 text-white/30 group-hover:text-red-500 mb-2" />
          <span className="text-xs text-white/50 group-hover:text-white">Change Video</span>
          ...
          <AnimatePresence>
            {isRecording && (
              <motion.div 
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="absolute inset-0 bg-[#0a0a0a]/90 backdrop-blur-xl flex flex-col items-center justify-center z-50"
              >
